...
class TellerTreasury(Document):
	def validate(self):
		if self.branch:
			...
			self.branch = branch_doc.name  # This ensures we use the branch ID in the name
	...
	def validate_accounts(self):
		...
		if hasattr(self, 'egy_account') and self.egy_account:
			...
			if account.account_type not in ['Bank', 'Cash']:
				frappe.throw(f"EGY Account must be of type Bank or Cash. Current type: {account.account_type}")
			...
			if self.egy_account not in branch_accounts:
				frappe.throw(f"EGY Account must belong to branch {self.branch}")
